...
<div class="col variation-block" id="variation-template" transition:slide|global>
	<div class="card mt-3 text-center mx-auto" style="width: 14rem">
		...
		<div class="card-header py-0">
			...
			<i class="bi bi-trash position-absolute text-danger end-0 top-0 pe-3 pt-1 clickable" on:click={deleteCallback}> </i>
		</div>
			...
			{#each internalOptions as option}
				<div class="row variation-item-block" transition:slide>
					...
					<div class="col">
						<!-- svelte-ignore a11y-click-events-have-key-events -->
						<!-- svelte-ignore a11y-no-static-element-interactions -->
						<i class="bi bi-x d-inline position-static text-secondary fs-3 clickable"
                        on:click={() => deleteVariation(option)}></i>
					</div>
				</div>
			{/each}
			<EditableSpan
				...
				on:onsave={(event) => {
					console.log(event);
					internalOptions = [...internalOptions, {name: event.detail.value, id: Date.now().toString()}];
					placeholderValue = '';
				}}
			/>
		</div>
